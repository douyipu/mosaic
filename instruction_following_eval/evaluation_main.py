...
def main(argv):
  ...
  all_inputs = evaluation_lib.read_prompt_list(_INPUT_DATA.value)
  prompt_to_response = evaluation_lib.read_prompt_to_response_dict(
      _INPUT_RESPONSE_DATA.value)

  inputs = [inp for inp in all_inputs if inp.prompt in prompt_to_response]
  skipped = len(all_inputs) - len(inputs)
  if skipped:
    logging.warning("Skipped %d prompts with no matching response", skipped)
  if not inputs:
    raise app.UsageError("No matching prompts between input_data and input_response_data")
